...
void update_position(int x, int y, int type, int breeding, int breeding_flag, int starvation, int starvation_flag){
  animal_world[x][y]->type = type;
  animal_world[x][y]->breeding_period = breeding;
  animal_world[x][y]->breeding_flag = breeding_flag;
  animal_world[x][y]->starvation_flag = starvation_flag;
  animal_world[x][y]->starvation_period = starvation;
}
...
void move(int num_generation, int type, int x, int y) {
  ...
  if( type == SQUIRREL || type == SQUIRREL_TREE) {
    ...
    if((y - 1 >= 0 ) && (animal_world[x][y-1]->type == EMPTY || animal_world[x][y-1]->type == TREE)) {
      array[sum].x=x;
      array[sum].y=y-1;
      array[sum].on_tree = animal_world[x][y-1]->type == TREE ? 1 : 0;
      sum++;
    }
    if((x + 1 < world_size) && (animal_world[x+1][y]->type == EMPTY || animal_world[x+1][y]->type == TREE)) {
      array[sum].x=x+1;
      array[sum].y=y;
      array[sum].on_tree = animal_world[x+1][y]->type == TREE ? 1 : 0;
      sum++;
    }
    ...
    if( sum == 0 ){
      ...
      if(animal_world[x][y]->breeding_period == 0 ) {
        ...
        if(array[next].on_tree) {
          update_position(array[next].x, array[next].y, SQUIRREL_TREE, animal_world[x][y]->breeding_period - 1, num_generation, 0, 0);
        }
        else {
    ...
    if(sum == 1) {
      if(animal_world[x][y]->breeding_period == 0 ) {
        ...
        update_position(array[0].x, array[0].y, WOLF, animal_world[x][y]->breeding_period - 1, num_generation, animal_world[x][y]->starvation_period - 1, num_generation);
        update_position(x,y, EMPTY, 0, 0, 0, 0);
      }
      ...
      if(animal_world[x][y]->breeding_period == 0 ) {
        ...
        update_position(array[next].x, array[next].y, WOLF, animal_world[x][y]->breeding_period - 1, num_generation, animal_world[x][y]->starvation_period - 1, num_generation);
        update_position(x,y, EMPTY, 0, 0, 0, 0);
      }
    } else {
      sum = 0;
